...
import {
  ...
  getAuctionData,
  getCharacterData,
  getTodayExportaionIsland,
  getUserSubCharacter,
  getWeeklyAbyssDungeouns,
  getWeeklyAbyssGuardians,
  itemCraftPrinceing,
  persistMarketData,
} from "./utils/axiosLostarkApi";
import {
  argCheck,
  argNumCheck,
  merchantTimeIndex,
...
import {
  ...
  SingleTextBox,
  TooltipHeader,
} from "./types/loaApiEquipTooltips";

export const functionSwithcer = async (
  msg: string,
  ...arg: string[]
  ...
  switch (msg) {
    ...
    case "프로키온": {
      ...
      eventList.forEach((event) => {
        responseText += `${event.name}(${event.startTime
          .map((x) => x.getHours())
          .join("시, ")}시): ${event.reward}\n`;
      });
      return [
        {
          type: "text",
    ...
    case "장비": {
      ...
      info.ArmoryEquipment.slice(0, 6).forEach((equip) => {
        ...
        if (FullHeader.Element_000?.value !== undefined) {
          ...
          const extractResult = itemNameExtractor.exec(
            FullHeader.Element_000?.value
          );
          extractResult !== null
            ? (itemDescription += `${extractResult[1]}`)
            : "아이템 미장착";
        } else {
          return;
        }
        ...
        if (FullHeader.Element_001?.value.leftStr2 !== undefined) {
          ...
          const extractResult = itemLeveLExtractor.exec(
            FullHeader.Element_001?.value.leftStr2
          );
          extractResult !== null
            ? (itemDescription += `\n(${extractResult[1]}레벨)`)
            : "(레벨 미표기)";
        ...
        const setLevelParse = /.{2} <.*?>(.+?)<\/.*?>/.exec(
          (tooltips[tooltipKey] as ItemPartBox).value.Element_001
        );
        ...
        //상재
        if ((FullHeader.Element_005 as SingleTextBox)?.value !== undefined) {
          let upperReforgeString = (
            FullHeader.Element_005 as SingleTextBox
          )?.value
            .replace(/<.*?>/gi, "")
            .replace(/\[.*?\]/gi, "");
        ...
        elementNames.forEach((key) => {
          ...
          if (
            etcTooltips.value["Element_000"]?.topStr !== undefined &&
            etcTooltips.value["Element_000"]?.topStr.includes("초월")
          ) {
            const targetStr = etcTooltips.value["Element_000"]?.topStr;
            const imgRemovedStr = targetStr.replace(/<img.*<\/img>/gim, "");
            const activationCheck = /<font color='#(.{6})'>/i.exec(
              imgRemovedStr
            );
            if (activationCheck === null) return;
            if (activationCheck[1] === "787878") return;
            const refinedStr = imgRemovedStr
              .replace(/<.*?>/gi, "")
              .replace("[초월] ", "");
            ...
            etcTooltips.value["Element_000"]?.topStr !== undefined &&
            etcTooltips.value["Element_000"]?.topStr.includes("엘릭서")
          ) {
            const etcKeyList = Object.getOwnPropertyNames(
              etcTooltips.value["Element_000"].contentStr
            );

            etcKeyList.forEach((etcKey) => {
              const targetStr =
                etcTooltips.value["Element_000"]?.contentStr[etcKey]
                  ?.contentStr;
              const imgRemovedStr = targetStr?.replace(/<img.*<\/img>/gim, "");
              ...
              if (activationCheck === null) return;
              if (activationCheck[1] === "787878") return;
              const refinedStr = imgRemovedStr.replace(/<.*?>/gi, "");
    ...
    case "악세": {
      ...
      const access: Equipment[] = info.ArmoryEquipment.filter((element) =>
        ["목걸이", "귀걸이", "반지"].find((trgarr) => trgarr === element.Type)
      );
      if (access.length === 0) {
        return [
          {
      ...
      access.forEach((equip) => {
        ...
        const content = JSON.stringify(
          JSON.parse(equip.Tooltip)["Element_006"]["value"]["Element_000"][
            "contentStr"
          ]
        );
        const matches = content.matchAll(
          /\[<FONT COLOR='\#.{6}'>(.{1,10})<\/FONT>] 활성도 \+(.)<BR>"/gm
        );
    ...
    case "초월": {
      ...
      info.ArmoryEquipment.slice(1, 6).forEach((equip) => {
        ...
        if (FullHeader.Element_000?.value !== undefined) {
          ...
          switch (true) {
            case equipmentName.includes("머리") ||
              equipmentName.includes("투구") ||
              equipmentName.includes("모자"):
              itemDescription += "머리: ";
              break;

            case equipmentName.includes("견갑") ||
              equipmentName.includes("어깨"):
              itemDescription += "어깨: ";
              break;
    ...
    case "정보": {
      ...
      const thumnail =
        info.ArmoryProfile.CharacterImage === null
          ? "https://img1.daumcdn.net/thumb/R1280x0/?scode=mtistory2&fname=https%3A%2F%2Fblog.kakaocdn.net%2Fdn%2Fb77pWc%2Fbtr9Ox1B2fy%2F7XrjsZ2fc3tBI4s4U3WN1K%2Fimg.png"
          : info.ArmoryProfile.CharacterImage;
      const summary = `${info.ArmoryProfile.ItemMaxLevel} / ${
        info.ArmoryEquipment === null
          ? "장비없음"
          : info.ArmoryEquipment[0]?.Name
      }`;
      const statList =
        info.ArmoryProfile.Stats === null
          ? "만들어만 둔 캐릭"
          : info.ArmoryProfile.Stats.filter((x) =>
              ["치명", "특화", "신속"].includes(x.Type)
            )
      ...
      const envList =
        info.ArmoryEngraving === null
          ? "각인없음"
          : info.ArmoryEngraving.Effects.map((engrave) => engrave.Name).join(
              ", "
            );
      const gemList =
        info.ArmoryGem === null
          ? "-쌀-"
          : info.ArmoryGem.Gems.map((gem) => gem.Level).join(", ");
      const tripodList =
        info.ArmorySkills === null
          ? "스킬미장착"
          : info.ArmorySkills.filter((skill) => skill.Level > 1)
              .map((skill) =>
                skill.Tripods.filter((tripod) => tripod.Level > 1)
              )
              .flatMap((x) => x)
              .map((tripod) => tripod.Level)
              .join(", ");
      const description =
        info.ArmoryCard === null
          ? "카드 미장착"
          : `${
              info.ArmoryCard?.Effects[0]?.Items[
                info.ArmoryCard?.Effects[0]?.Items?.length - 1
              ]?.Name
            }: ${
              info.ArmoryCard.Effects[0]?.Items[
                info.ArmoryCard.Effects[0]?.Items?.length - 1
    ...
    case "부캐": {
      ...
      sibilingList.forEach((server) => {
        ...
        server.characterList.forEach((characterInfo) => {
          reusltText =
            reusltText +
            `${characterInfo.classname} ${characterInfo.name} - Lv.${characterInfo.level}\n`;
        });
      });
      ...
      return [
        {
          ...
          body:
            ...
            sibilingList
              .map((server) => {
                const serverSum = server.characterList
                  .sort((fore, back) => {
                  ...
                  .reduce((pre, cur) => {
                    return (
                      pre +
                      weeklyProfitCalc(
                        Number.parseInt(String(cur.level).replace(",", ""))
                      )
                    );
                  }, 0);
    ...
    case "떠상": {
      ...
      const korlarkResult = await axios.get<korlarkResponse>(
        `https://api.korlark.com/merchants?limit=15&server=${targetServer}`
      );
      ...
      korlarkResult.data.merchants.forEach((row) => {
        if (
          timeset ===
          merchantTimeIndex(
            (new Date(Date.parse(row.created_at)).getHours() + 9) % 24
          )
        ) {
